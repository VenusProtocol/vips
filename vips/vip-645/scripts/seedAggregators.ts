/**
 * Seeds the VIP-645 grant / revoke batches onto a chain's ACMCommandsAggregator.
 *
 ...
 * vips/vip-645/utils/commands.ts. Revokes are split into batches (buildRevokeBatches) so no seed tx exceeds
 * the Osaka per-tx gas cap; each batch lands at its own index.
 *
 *   npx hardhat run vips/vip-645/scripts/seedAggregators.ts --network <chain>
...
async function main() {
  ...
  if (!(chain in AGGREGATOR)) throw new Error(`VIP-645 has no aggregator for network "${chain}"`);
