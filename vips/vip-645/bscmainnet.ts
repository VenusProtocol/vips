...
import { BNB_CORE } from "../vip-634/phase4Markets";
import { PART1_POOLS, generateCoreEmodeCommands, generateStep2Commands } from "./deprecationStep2";

const meta = {
  version: "v2",
  title: "VIP-645 Market Deprecation Phase 4 (Step 2, Part 1 of 2)",
  description: `#### Summary

This proposal is the first of two VIPs implementing Step 2 — the final step — of the wind-down of deprecated and off-boarded markets across BNB Chain, opBNB, Optimism, Unichain, Ethereum, Base, Arbitrum and zkSync Era. Step 1 ([VIP-634](https://community.venus.io/t/deprecate-venus-core-on-opbnb-optimism-unichain-isolated-pools/5760) and [VIP-635](https://community.venus.io/t/may-2026-risk-parameter-update-asset-off-boarding/5785)) set reserve factors to 100%, repointed each market to a deprecation interest rate model, and closed the residual supply / borrow cap and collateral-factor gaps. Step 2 sets the liquidation threshold of every market in scope to zero, fully removing these assets as collateral.

As in Step 1, the work is split across two VIPs to respect the BNB Chain per-transaction gas cap. Part 1 covers the BNB Core pool, the BNB BTC / DeFi / GameFi isolated pools, and the opBNB, Optimism, Unichain and Ethereum Core deployments. Part 2 (VIP-646) covers the remaining pools.
...
- **VIP execution simulation**: validated in a fork environment that each in-scope market ends with a zero collateral factor and a zero liquidation threshold, on both the isolated-pool Comptrollers and the BNB Core diamond (base pool and e-mode pools).
...
export const vip645 = () =>
  makeProposal(
    [
      // ── BNB Core (diamond): zero the base pool (poolId 0) threshold of every market …
      ...generateStep2Commands(BNB_CORE),
      // … and the e-mode pool threshold for the markets that also live in an e-mode pool.
      ...generateCoreEmodeCommands(),

      // ── Remaining Part 1 pools: BNB BTC / DeFi / GameFi, then opBNB, Optimism,
      //    Unichain and Ethereum Core. ──
      ...PART1_POOLS.filter(pool => pool !== BNB_CORE).flatMap(generateStep2Commands),
    ],
    meta,
