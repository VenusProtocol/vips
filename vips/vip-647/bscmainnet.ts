...
2. **Oracle price-feed update.** Repoints the price feed of the listed assets on Ethereum, Arbitrum and BNB Chain by updating the feed inside each asset's MAIN oracle adapter (\`setTokenConfig(asset, feed, maxStalePeriod)\`). Each \`maxStalePeriod\` is set to the new Chainlink feed's published heartbeat plus a latency cushion. On BNB Chain, native BNB is configured separately from WBNB (each under its own underlying), and THE is moved onto the ChainlinkOracle adapter — its new feed being a Chainlink feed — with the ResilientOracle MAIN slot repointed to that adapter and its previous RedStone configuration enabled as the FALLBACK oracle.
