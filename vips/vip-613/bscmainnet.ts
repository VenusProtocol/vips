...
export const vip613 = () => {
  const meta = {
    version: "v2",
    title: "VIP-613 [BNB Chain] DeviationSentinel Expansion — Emergency Brake Step 1",
    description: `#### Summary

This VIP expands **DeviationSentinel** oracle-deviation monitoring on BNB Chain from a single market (CAKE) to **25 Core Pool markets** at a unified **10%** deviation threshold via the PancakeSwapOracle deployed in [VIP-590](https://venus.io/#/governance/proposal/590?chainId=56) and rewired through EBrakeV2 in [VIP-610](https://venus.io/#/governance/proposal/610?chainId=56).

#### Context

Deviation Sentinel is Venus V4's oracle manipulation defense system. It compares ResilientOracle prices against PancakeSwap V3 spot prices and triggers automatic circuit breakers when deviations exceed configured thresholds. Currently only CAKE is configured (threshold = 20%). This VIP expands coverage to 25 BSC Core Pool markets with a unified 10% threshold.

#### Per-Market Actions (×25)

For each of the 25 markets, the VIP performs three configuration calls:

- PancakeSwapOracle.setPoolConfig(token, pool) — bind underlying token to its PancakeSwap V3 pool
- SentinelOracle.setTokenOracleConfig(token, PancakeSwapOracle) — route SentinelOracle price queries to PancakeSwapOracle
- DeviationSentinel.setTokenConfig(token, (10, true)) — 10% deviation threshold, monitoring enabled

All three permissions were granted to the Normal Timelock in VIP-590; no additional ACM permission changes required.

#### Markets Configured (25)
...
- **CAKE** — CAKE/WBNB (0.25%) — 0x133b3d95bad5405d14d53473671200e9342896bf (was 20%, now 10%)
