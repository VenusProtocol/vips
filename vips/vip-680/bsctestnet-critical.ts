...
- Testnet-only. The Hub is not yet deployed on any mainnet.`,
  ...
  return makeProposal(
    [
      ...HUB_GOVERNANCE.map(sig => giveCallPermission(HUB_USDT, sig, CRITICAL_TIMELOCK)),
      ...CORE_FLUX_GOVERNANCE.map(sig => giveCallPermission(CORE_SOURCE_USDT, sig, CRITICAL_TIMELOCK)),
      ...FRV_GOVERNANCE.map(sig => giveCallPermission(FRV_SOURCE_USDT, sig, CRITICAL_TIMELOCK)),
