...
import {
  ...
  generateCommands,
  orchestrate,
} from "../scripts/generateSafePauseJson";
...
describe("generateSafePauseJson", () => {
  ...
  describe("exportJson", () => {
    ...
    it("13: normal commands → returns ExportResult, mkdirSync called 1x, writeFileSync called 2x (txBuilder, metadata)", async () => {
      const commands = [
      ...
      expect((deps.mkdirSync as sinon.SinonStub).callCount).to.be.equal(1); // OUTPUT_DIR
      expect((deps.writeFileSync as sinon.SinonStub).callCount).to.equal(2); // txBuilder, metadata
    });
  ...
  describe("orchestrate", () => {
    ...
    function makeOrcDeps(): OrchestrateDeps & { stubs: { gen: sinon.SinonStub; exp: sinon.SinonStub } } {
      ...
      return {
        ...
        exportJson: exp,
        stubs: { gen, exp },
      } as unknown as OrchestrateDeps & { stubs: { gen: sinon.SinonStub; exp: sinon.SinonStub } };
    }

    it("17: bscmainnet + pause → generateCommands('pause'), exportJson with Guardian", async () => {
    ...
    it("18: bscmainnet + cf_zero → generateCommands('cf_zero'), exportJson with Critical Guardian", async () => {
    ...
    it("19: bscmainnet + both → 2x generateCommands (pause + cf_zero), 2x exportJson (Guardian + Critical Guardian)", async () => {
    ...
    it("20: ethereum + pause → generateCommands('pause'), exportJson with Guardian", async () => {
    ...
    it("21: ethereum + cf_zero → generateCommands('cf_zero'), exportJson with Guardian", async () => {
    ...
    it("22: ethereum + both → generateCommands('pause') and generateCommands('cf_zero'), single exportJson with Guardian", async () => {
