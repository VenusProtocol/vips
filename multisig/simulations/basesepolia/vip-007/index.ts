...
import vip006 from "../../../proposals/basesepolia/vip-006";
import vip007, {
  BOUND_VALIDATOR,
  COMPTROLLERS,
  COMPTROLLER_BEACON,
  ...
  XVS_STORE,
} from "../../../proposals/basesepolia/vip-007";
...
forking(18696318, async () => {
  ...
  describe("Pre-VIP behavior", async () => {
    before(async () => {
      ...
      poolRegistry = new ethers.Contract(basesepolia.POOL_REGISTRY, POOL_REGISTRY_ABI, provider);
      xvsVault = new ethers.Contract(basesepolia.XVS_VAULT_PROXY, XVS_VAULT_ABI, provider);
      ...
      treasury = await ethers.getContractAt(TREASURY_ABI, NETWORK_ADDRESSES.basesepolia.VTREASURY);
    });

    it("owner of proxy admin is guardian", async () => {
      expect(await proxyAdmin.owner()).to.equal(basesepolia.GUARDIAN);
    });
    ...
    it("owner of ComptrollerBeacon is guardian", async () => {
      expect(await comptrollerBeacon.owner()).to.equal(basesepolia.GUARDIAN);
    });

    it("owner of VTokenBeacon is guardian", async () => {
      expect(await vTokenBeacon.owner()).to.equal(basesepolia.GUARDIAN);
    });
  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("owner of proxy admin is timelock", async () => {
      expect(await proxyAdmin.owner()).to.equal(basesepolia.NORMAL_TIMELOCK);
    });

    it("pending owner", async () => {
      expect(await prime.pendingOwner()).to.equal(basesepolia.NORMAL_TIMELOCK);
    ...
    for (const comptrollerAddress of COMPTROLLERS) {
      it(`correct pending owner for ${comptrollerAddress}`, async () => {
        const c = new ethers.Contract(comptrollerAddress, COMPTROLLER_ABI, provider);
        expect(await c.pendingOwner()).to.equal(basesepolia.NORMAL_TIMELOCK);
      });
    }
    ...
    for (const vTokenAddress of VTOKENS) {
      it(`correct pending owner for ${vTokenAddress}`, async () => {
        const v = new ethers.Contract(vTokenAddress, VTOKEN_ABI, provider);
        expect(await v.pendingOwner()).to.equal(basesepolia.NORMAL_TIMELOCK);
      });
    }

    it("owner of ComptrollerBeacon is NT", async () => {
      expect(await comptrollerBeacon.owner()).to.equal(basesepolia.NORMAL_TIMELOCK);
    });

    it("owner of VTokenBeacon is NT", async () => {
      expect(await vTokenBeacon.owner()).to.equal(basesepolia.NORMAL_TIMELOCK);
    });

    it("pending owner of PoolRegistry is NT", async () => {
      expect(await poolRegistry.pendingOwner()).to.equal(basesepolia.NORMAL_TIMELOCK);
    });
    ...
    it("Should set bridge pending owner to Normal Timelock", async () => {
      ...
      expect(pendingOwner).equals(basesepolia.NORMAL_TIMELOCK);
    });
    it("Should set XVS owner to Normal Timelock", async () => {
      const owner = await xvs.owner();
      expect(owner).equals(basesepolia.NORMAL_TIMELOCK);
    });
    it("correct pending owner", async () => {
    ...
    it("Should set pendingOwner to Normal Timelock", async () => {
      ...
      expect(pendingOwner).equals(NETWORK_ADDRESSES.basesepolia.NORMAL_TIMELOCK);
    });
  });
});
