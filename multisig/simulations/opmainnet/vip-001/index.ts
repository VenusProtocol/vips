...
} from "../../../proposals/opmainnet/vip-001";
...
forking(125555900, async () => {
  ...
  describe("Pre-Execution state", () => {
    ...
    it("Trusted remote should not exist for any network(bscmainnet, opbnbmainnet, ethereum)", async () => {
      ...
      await expect(xvsBridge.getTrustedRemoteAddress(ARBITRUM_ONE_ENDPOINT_ID)).to.be.revertedWith(
        "LzApp: no trusted path record",
      );
      await expect(xvsBridge.getTrustedRemoteAddress(ZKSYNC_MAINNET_ENDPOINT_ID)).to.be.revertedWith(
        "LzApp: no trusted path record",
      );
    });
  ...
  describe("Post-Execution state", () => {
    ...
    it("Should set trusted remote address in bridge for all five networks", async () => {
      ...
      trustedRemote = await xvsBridge.getTrustedRemoteAddress(ARBITRUM_ONE_ENDPOINT_ID);
      expect(trustedRemote).equals(ARBITRUM_ONE_TRUSTED_REMOTE);

      trustedRemote = await xvsBridge.getTrustedRemoteAddress(ZKSYNC_MAINNET_ENDPOINT_ID);
      expect(trustedRemote).equals(ZKSYNC_MAINNET_TRUSTED_REMOTE);
    });
    ...
    it("Should set correct max daily limit for all five networks", async () => {
      ...
      limit = await xvsBridge.chainIdToMaxDailyLimit(ARBITRUM_ONE_ENDPOINT_ID);
      expect(limit).equals(MAX_DAILY_SEND_LIMIT);

      limit = await xvsBridge.chainIdToMaxDailyLimit(ZKSYNC_MAINNET_ENDPOINT_ID);
      expect(limit).equals(MAX_DAILY_SEND_LIMIT);
    });
    ...
    it("Should set correct max single limit for all five networks", async () => {
      ...
      limit = await xvsBridge.chainIdToMaxSingleTransactionLimit(ARBITRUM_ONE_ENDPOINT_ID);
      expect(limit).equals(SINGLE_SEND_LIMIT);

      limit = await xvsBridge.chainIdToMaxSingleTransactionLimit(ZKSYNC_MAINNET_ENDPOINT_ID);
      expect(limit).equals(SINGLE_SEND_LIMIT);
    });
    ...
    it("Should set correct max daily receive limit for all five networks", async () => {
      ...
      limit = await xvsBridge.chainIdToMaxDailyReceiveLimit(ARBITRUM_ONE_ENDPOINT_ID);
      expect(limit).equals(MAX_DAILY_RECEIVE_LIMIT);

      limit = await xvsBridge.chainIdToMaxDailyReceiveLimit(ZKSYNC_MAINNET_ENDPOINT_ID);
      expect(limit).equals(MAX_DAILY_RECEIVE_LIMIT);
    });
    ...
    it("Should set correct max single receive limit for all five networks", async () => {
      ...
      limit = await xvsBridge.chainIdToMaxSingleReceiveTransactionLimit(ARBITRUM_ONE_ENDPOINT_ID);
      expect(limit).equals(SINGLE_RECEIVE_LIMIT);

      limit = await xvsBridge.chainIdToMaxSingleReceiveTransactionLimit(ZKSYNC_MAINNET_ENDPOINT_ID);
      expect(limit).equals(SINGLE_RECEIVE_LIMIT);
    });
    ...
    it("Should set correct min destination gas", async () => {
      ...
      limit = await xvsBridge.minDstGasLookup(ARBITRUM_ONE_ENDPOINT_ID, 0);
      expect(limit).equals(MIN_DEST_GAS);

      limit = await xvsBridge.minDstGasLookup(ZKSYNC_MAINNET_ENDPOINT_ID, 0);
      expect(limit).equals(MIN_DEST_GAS);
    });
  });
