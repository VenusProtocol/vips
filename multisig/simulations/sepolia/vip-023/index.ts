...
import {
  ...
  PTweETH,
  vPTweETH
} from "../../../proposals/sepolia/vip-023";
...
forking(5673551, () => {
  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("check price", async () => {
      expect(await resilientOracle.callStatic.getPrice(PTweETH)).to.be.closeTo(parseUnits("3700", 18), parseUnits("50", 18));
      // expect(await resilientOracle.getUnderlyingPrice(vPTweETH)).to.be.closeTo(parseUnits("3700", 18), parseUnits("50", 18));
    });
