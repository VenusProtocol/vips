...
forking(16716512, () => {
  ...
  before(async () => {
    ...
    xvsStore = await ethers.getContractAt(XVS_STORE_ABI, opbnbtestnet.XVS_STORE);
  ...
  describe("Post tx checks", () => {
    describe("Generic checks", async () => {
      before(async () => {
        ...
        const admin = await initMainnetUser(opbnbtestnet.NORMAL_TIMELOCK, ethers.utils.parseEther("1"));
        ...
        await xvs.connect(xvsHolder).transfer(opbnbtestnet.XVS_STORE, ethers.utils.parseEther("1"));
        await mine(604800);
      });
      checkXVSVault();
    });
    ...
    it("Should set correct xvs store address", async () => {
      ...
      expect(xvsStore).equals(opbnbtestnet.XVS_STORE);
    });
