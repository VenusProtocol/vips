...
forking(22581612, async () => {
  ...
  describe("Pre-VIP behavior", async () => {
    before(async () => {
      ...
      poolRegistry = new ethers.Contract(opsepolia.POOL_REGISTRY, POOL_REGISTRY_ABI, provider);
      ...
      treasury = await ethers.getContractAt(TREASURY_ABI, NETWORK_ADDRESSES.opsepolia.VTREASURY);
    });
    ...
    it("owner of proxy admin is guardian", async () => {
      expect(await proxyAdmin.owner()).to.equal(opsepolia.GUARDIAN);
    });
    ...
    it("owner of ComptrollerBeacon is guardian", async () => {
      expect(await comptrollerBeacon.owner()).to.equal(opsepolia.GUARDIAN);
    });

    it("owner of VTokenBeacon is guardian", async () => {
      expect(await vTokenBeacon.owner()).to.equal(opsepolia.GUARDIAN);
    });
  ...
  describe("Post-VIP behavior", async () => {
    ...
    for (const ntg of NTGs) {
      it(`should have no pending owner for ${ntg}`, async () => {
        const c = new ethers.Contract(ntg, NTG_ABI, provider);
        expect(await c.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
      });
    }

    it("owner of proxy admin is timelock", async () => {
      expect(await proxyAdmin.owner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });

    it("pending owner", async () => {
      expect(await prime.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
      expect(await plp.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });

    it("pending owner of psr", async () => {
      expect(await psr.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });
    ...
    for (const vTokenAddress of VTOKENS) {
      it(`correct pending owner for ${vTokenAddress}`, async () => {
        const v = new ethers.Contract(vTokenAddress, VTOKEN_ABI, provider);
        expect(await v.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
      });
    }

    it("owner of ComptrollerBeacon is NT", async () => {
      expect(await comptrollerBeacon.owner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });

    it("owner of VTokenBeacon is NT", async () => {
      expect(await vTokenBeacon.owner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });

    it("pending owner of PoolRegistry is NT", async () => {
      expect(await poolRegistry.pendingOwner()).to.equal(opsepolia.NORMAL_TIMELOCK);
    });
    ...
    it("Should set bridge pending owner to Normal Timelock", async () => {
      ...
      expect(pendingOwner).equals(opsepolia.NORMAL_TIMELOCK);
    });
    ...
    it("Should set pendingOwner to Normal Timelock", async () => {
      ...
      expect(pendingOwner).equals(NETWORK_ADDRESSES.opsepolia.NORMAL_TIMELOCK);
    });
  });
});
