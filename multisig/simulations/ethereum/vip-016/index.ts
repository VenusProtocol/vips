...
forking(19638382, () => {
  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("check price", async () => {
      expect(await resilientOracle.getPrice(weETH)).to.be.closeTo(parseUnits("3640", 18), parseUnits("5", 18));
    });

    it("check non-equivalence oracle", async () => {
      expect(await nonEquivalenceOracle.getPrice(weETH)).to.be.closeTo(parseUnits("3640", 18), parseUnits("5", 18));
    });
