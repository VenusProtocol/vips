...
import {
  ...
  VTREASURY,
  vip005,
} from "../../../proposals/vip-005/vip-005-ethereum";
    ...
    it("Verify Multisig permissions for PSR", async () => {
      expect(
        await accessControlManager
          .connect(psrSigner)
          .isAllowedToCall(ETHEREUM_MULTISIG, "addOrUpdateDistributionConfigs(DistributionConfig[])"),
      ).to.be.true;

      expect(
        await accessControlManager
          .connect(psrSigner)
          .isAllowedToCall(ETHEREUM_MULTISIG, "removeDistributionConfig(Schema,address)"),
      ).to.be.true;
    });
