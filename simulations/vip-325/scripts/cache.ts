...
type BigNumberToString<T> = T extends BigNumber ? string : { [P in keyof T]: T[P] extends BigNumber ? string : T[P] };
...
export class Cache<ValueType> {
  ...
  protected static async createFromFile<ValueType>(
    ...
    try {
      ...
      // treat as no cache if there are errors in the file
    }
  ...
  readKeys(keys: ReadonlyArray<string>) {
    return this.decodeChunk(keys.flatMap(key => (this._kv[key] !== undefined ? [[key, this._kv[key]]] : [])));
  }
...
export class BorrowBalancesCache extends Cache<BigNumber> {
  constructor(fileName: string, kv: Record<string, string>) {
    super(fileName, kv, BorrowBalancesCache.encodeBigNumber, BorrowBalancesCache.decodeBigNumber);
  }
