...
forking(132849860, async () => {
  ...
  describe("Pre-VIP behavior", async () => {
    ...
    it("WBETH price read reverts (stale Binance feed)", async () => {
  ...
  describe("Post-VIP behavior", async () => {
    it("prices WBETH at the direct price", async () => {
      expect(await resilientOracle.getPrice(WBETH)).to.equal(WBETH_DIRECT_PRICE);
    });
