...
forking(4761402, async () => {
  ...
  describe("vTokens deployment", () => {
    before(async () => {
      await setMaxStalePeriodInChainlinkOracle(CHAINLINK_ORACLE, newMarket.vToken.underlying.address, ETH_USD_FEED, NORMAL_TIMELOCK);
    });
  ...
  describe("Post-VIP state", () => {
    ...
    describe("PoolRegistry state", () => {
      ...
      it(`should register ${newMarket.vToken.symbol} in PoolRegistry`, async () => {
        const registeredVToken = await poolRegistry.getVTokenForAsset(COMPTROLLER_CORE, newMarket.vToken.underlying.address);
