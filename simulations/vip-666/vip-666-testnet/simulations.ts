...
forking(68436663, async () => {
  ...
  describe("Pre-VIP behavior", () => {
    ...
    it("Check the before state of risk params for sUSDe, USDe, PT_USDe", async () => {
      ...
      expect(info.collateralFactorMantissa).to.equal(parseUnits("0.7875", 18));
      expect(info.liquidationThresholdMantissa).to.equal(parseUnits("0.825", 18));

      info = await unitroller.poolMarkets(1, vUSDe);
      expect(info.collateralFactorMantissa).to.equal(parseUnits("0", 18));
      expect(info.liquidationThresholdMantissa).to.equal(parseUnits("0", 18));

      info = await unitroller.poolMarkets(1, vPT_USDe_30Oct2025);
      expect(info.collateralFactorMantissa).to.equal(parseUnits("0.9", 18));
