    ...
    for (const m of MARKETS) {
      describe(`${m.vToken.symbol} market`, async () => {
        ...
        it("wires the capped oracle to the vault and its ERC4626 asset", async () => {
          ...
          expect(await cappedOracle.RESILIENT_ORACLE()).to.equal(bsctestnet.RESILIENT_ORACLE);
        });
        ...
        it("should leave no underlying approval to the vToken", async () => {
          expect(await underlying.allowance(bsctestnet.NORMAL_TIMELOCK, m.vToken.address)).to.equal(0);
        });
