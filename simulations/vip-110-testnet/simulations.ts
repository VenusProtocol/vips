...
import { vip110Testnet } from "../../vips/vip-110-testnet";
...
forking(29020724, () => {
  ...
  testVip("VIP-110 Change Oracle and Configure Resilient Oracle", vip110Testnet(), {
  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("validate vToken prices", async () => {
      for (let i = 0; i < vTokens.length; i++) {
        ...
        if (vToken.name === "vXVS") {
          expect(price).to.be.equal(parseUnits("7.669", 18)); // in testnet XVS price is returned wrong by old oracle
        } else {
          expect(price).to.be.equal(parseUnits(vToken.price, 18));
        }
      }
