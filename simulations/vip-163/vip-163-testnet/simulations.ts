...
forking(32725445, () => {
  ...
  testVip("VIP-163Testnet Add THE Market", vip163Testnet(), {
  ...
  describe("Post-VIP state", () => {
    describe("PoolRegistry state", () => {
      ...
      it("should register in PoolRegistry", async () => {
        const vToken = await poolRegistry.getVTokenForAsset(DEFI_COMPTROLLER, THE);
    ...
    describe("Basic supply/borrow/repay/redeem scenario", () => {
      ...
      it("should be possible to enable the as collateral", async () => {
        await comptroller.connect(user).enterMarkets([VTHE_DeFi]);
        expect(await comptroller.getAssetsIn(user.address)).to.deep.equal([VTHE_DeFi]);
      });
