...
forking(55419109, async () => {
  ...
  describe("Pre-VIP behaviour", async () => {
    it("check BTCB price", async () => {
      expect(await resilientOracle.getPrice("0x7c6b91D9Be155A6Db01f749217d76fF02A7227F2")).to.equal(parseUnits("96881.65370663", 18));
    });

    it("check ETH price", async () => {
      expect(await resilientOracle.getPrice("0xE7798f023fC62146e8Aa1b36Da45fb70855a77Ea")).to.equal(parseUnits("1831.74800583", 18));
    });

    it("check FDUSD price", async () => {
      expect(await resilientOracle.getPrice("0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb")).to.equal(parseUnits("0.99875661", 18));
    });

    it("check USDT price", async () => {
      expect(await resilientOracle.getPrice("0x9e5AAC1Ba1a2e6aEd6b32689DFcF62A509Ca96f3")).to.equal(parseUnits("1.00059333", 18));
    });

    it("check WBNB price", async () => {
      expect(await resilientOracle.getPrice("0x4200000000000000000000000000000000000006")).to.equal(parseUnits("597.50660256", 18));
    });
  });
