...
const accounts = [
  ...
  "0x5d7e7022cec23df5c66c0fb89f886d6454de0ec9",
  ...
  "0x9eb199fea251abfa1f2633c29fc6bf4d8acdade4",
...
const getMarketDebts = async (market: MarketData) => {
...
const main = async () => {
  ...
  const unfilteredMarketDebtData = await Promise.all(
    markets.map(async market => {
      const debts = await getMarketDebts(market);
  ...
  const vaiDebts = await getVAIDebts();
