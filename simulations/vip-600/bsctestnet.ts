...
forking(85710269, async () => {
  ...
  describe("Pre-VIP behavior", () => {
    it("SwapRouter should have NORMAL_TIMELOCK as pending owner", async () => {
      expect(await swapRouter.pendingOwner()).to.equal(bsctestnet.NORMAL_TIMELOCK);
    });
  ...
  describe("Post-VIP behavior", () => {
    it("SwapRouter should have NORMAL_TIMELOCK as owner", async () => {
      expect(await swapRouter.owner()).to.equal(bsctestnet.NORMAL_TIMELOCK);
    });
