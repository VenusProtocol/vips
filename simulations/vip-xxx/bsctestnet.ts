...
import vipxxx, { COMPTROLLER, MARKET_CAP_RISK_STEWARD, ACCESS_CONTROL_MANAGER, ANY_TARGET_CONTRACT } from "../../vips/vip-xxx/bsctestnet";
...
forking(48650752, async () => {
  ...
  describe("Post-VIP behavior", () => {
    ...
    it("Market Cap Risk Steward should be able to set supply and borrow caps on markets", async () => {
      ...
      await expect(comptroller.connect(await ethers.getSigner(MARKET_CAP_RISK_STEWARD))
      ...
      await expect(isolatedPoolComptroller.connect(await ethers.getSigner(MARKET_CAP_RISK_STEWARD))
