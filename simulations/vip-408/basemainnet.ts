...
const FAST_TRACK_TIMELOCK = "0x209F73Ee2Fa9A72aF3Fa6aF1933A3B58ed3De5D7";
const CRITICAL_TIMELOCK = "0x47F65466392ff2aE825d7a170889F7b5b9D8e60D";
...
forking(23531762, async () => {
  ...
  before(async () => {
    executor = new ethers.Contract(
      basemainnet.OMNICHAIN_GOVERNANCE_EXECUTOR,
  ...
  describe("Pre-VIP behaviour", async () => {
    it("Normal Timelock has default admin role on base mainnet", async () => {
      const acm = await ethers.getContractAt(ACCESS_CONTROL_MANAGER_ABI, ACM);
      const hasRole = await acm.hasRole(DEFAULT_ADMIN_ROLE, basemainnet.NORMAL_TIMELOCK);
  ...
  describe("Post-VIP behaviour", async () => {
    ...
    it("check configuration", async () => {
      ...
      expect(await executor.proposalTimelocks(0)).equals(basemainnet.NORMAL_TIMELOCK);
      ...
      expect(await executor.trustedRemoteLookup(LzChainId.bscmainnet)).equals(
        ethers.utils.solidityPack(
          ["address", "address"],
          [getOmnichainProposalSenderAddress(), basemainnet.OMNICHAIN_GOVERNANCE_EXECUTOR],
        ),
      );
    ...
    it("Default admin role must be revoked from ACMAggregator contract on base mainnet", async () => {
    ...
    it("Guardian and all timelocks are allowed to call retryMessage ", async () => {
      ...
      expect(await acm.hasRole(roleHash, basemainnet.GUARDIAN)).to.be.true;
      expect(await acm.hasRole(roleHash, basemainnet.NORMAL_TIMELOCK)).to.be.true;
    ...
    it("Guardian is allowed to call forceResumeReceive but not timelocks", async () => {
      ...
      const roleHash = ethers.utils.keccak256(role);
      expect(await acm.hasRole(roleHash, basemainnet.GUARDIAN)).to.be.true;
      expect(await acm.hasRole(roleHash, basemainnet.NORMAL_TIMELOCK)).to.be.false;
    ...
    it("Normal Timelock is allowed to call setSendVersion but not other timelocks and guardian", async () => {
      ...
      const roleHash = ethers.utils.keccak256(role);
      expect(await acm.hasRole(roleHash, basemainnet.GUARDIAN)).to.be.false;
      expect(await acm.hasRole(roleHash, basemainnet.NORMAL_TIMELOCK)).to.be.true;
      expect(await acm.hasRole(roleHash, FAST_TRACK_TIMELOCK)).to.be.false;
      expect(await acm.hasRole(roleHash, CRITICAL_TIMELOCK)).to.be.false;
    });
