...
const BLOCK_NUMBER = 123557813;

const ERC20_ABI = ["function balanceOf(address) view returns (uint256)"];

forking(BLOCK_NUMBER, async () => {
  const provider = ethers.provider;
  const vTokenBeacon = new ethers.Contract(OPBNBMAINNET_VTOKEN_BEACON, VTOKEN_BEACON_ABI, provider);

  describe("Pre-VIP behaviour", () => {
    it("CORE_VTOKENS should cover all on-chain markets", async () => {
      const comptroller = new ethers.Contract(OPBNBMAINNET_CORE_COMPTROLLER, COMPTROLLER_ABI, provider);
      const allMarkets: string[] = await comptroller.getAllMarkets();
      expect(OPBNBMAINNET_CORE_VTOKENS.length).to.equal(allMarkets.length, "CORE_VTOKENS does not cover all markets");
    });
