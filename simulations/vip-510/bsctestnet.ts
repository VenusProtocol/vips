...
import { Actions, BNB_COMPTROLLERS, BNB_VTOKENS, vip510 } from "../../vips/vip-510/bsctestnet";
import COMPTROLLER_ABI from "./abi/Comptroller.json";
import OMNICHAIN_PROPOSAL_SENDER_ABI from "./abi/OmnichainProposalSender.json";

forking(52922185, async () => {
  ...
  testVip("VIP-510", await vip510(), {
    callbackAfterExecution: async txResponse => {
      await expectEvents(
        ...
        [60, 17, 0], // 0 events for "NewLiquidationThreshold" shows that there's no change in LiquidationThreshold.
      );
      await expectEvents(
        txResponse,
