...
forking(73267381, async () => {
  ...
  describe("Pre-VIP behaviour", async () => {
    describe("Old distribution speed", () => {
      it("has the old XVS distribution speed", async () => {
        const OLD_BSCTESTNET_XVS_PER_BLOCK_REWARD = parseUnits("0.0025", 18);
      ...
      it("has the old VAI vault rate", async () => {
        const OLD_BSCTESTNET_VAI_VAULT_RATE_PER_BLOCK = parseUnits("0.048", 18).toString();
      ...
      it("has the old BTCB distribution speed", async () => {
        const OLD_BSCTESTNET_BTCB_PER_BLOCK_REWARD = parseUnits("0.000000315393518518", 18);
      ...
      it("has the old ETH distribution speed", async () => {
        const OLD_BSCTESTNET_WETH_PER_BLOCK_REWARD = parseUnits("0.000006109664351851", 18);
      ...
      it("has the old USDC distribution speed", async () => {
        const OLD_BSCTESTNET_USDC_PER_BLOCK_REWARD = parseUnits("0.007", 6);
      ...
      it("has the old USDT distribution speed", async () => {
        const OLD_BSCTESTNET_USDT_PER_BLOCK_REWARD = parseUnits("0.007", 6);
    ...
    describe("Old Implementations & old block rate", () => {
      describe("Prime & PLP", () => {
        ...
        it("Prime should have block rate of 42048000", async () => {
          expect(await prime.blocksOrSecondsPerYear()).equals(42048000);
        });
        ...
        it("PLP should have block rate of 42048000", async () => {
          expect(await plp.blocksOrSecondsPerYear()).equals(42048000);
        });
      });
      ...
      describe("XVS Vault", () => {
        it("XVSVAULT should have block rate of 42048000", async () => {
          expect(await xvsVault.blocksOrSecondsPerYear()).equals(42048000);
        });
      });
        ...
        it("All Vtokens should have old block rate in IL", async () => {
          ...
          for (const pool of registeredPools) {
            ...
            for (const vtokenAddress of poolVTokens) {
              ...
              expect(await vtoken.blocksOrSecondsPerYear()).equals(42048000);
            }
          }
        });
      });
  ...
  describe("Post-VIP behavior", async () => {
    ...
    describe("New XVS emissions", () => {
      for (const oldSpeed of PREVIOUS_XVS_EMISSIONS) {
        it(`new supply-side XVS emissions per block for ${oldSpeed.symbol}`, async () => {
          const supplySpeed = await comptroller.venusSupplySpeeds(oldSpeed.market);
          expect(supplySpeed).to.equal(BigNumber.from(oldSpeed.supplySideSpeed).mul(100).div(167));
        });

        it(`new borrow-side XVS emissions per block for ${oldSpeed.symbol}`, async () => {
          const borrowSpeed = await comptroller.venusBorrowSpeeds(oldSpeed.market);
          expect(borrowSpeed).to.equal(BigNumber.from(oldSpeed.borrowSideSpeed).mul(100).div(167));
        });
      }
    });
    ...
    describe("Should point to new impl and have updated block rate", () => {
      describe("Prime & PLP", () => {
        ...
        it("Prime should have block rate of 70080000", async () => {
          expect(await prime.blocksOrSecondsPerYear()).equals(70080000);
        });
        ...
        it("PLP should have block rate of 70080000", async () => {
          expect(await plp.blocksOrSecondsPerYear()).equals(70080000);
        });
      });
      ...
      describe("VAI Controller", () => {
        ...
        it("VAI Controller should have block rate of 70080000", async () => {
          expect(await vaicontroller.getBlocksPerYear()).equals(70080000);
        });
      });
      ...
      describe("Shortfall", () => {
        ...
        it("Shortfall should have block rate of 70080000", async () => {
          expect(await shortfall.blocksOrSecondsPerYear()).equals(70080000);
        });
      });

      describe("XVS Vault", async () => {
        it("XVSVAULT should have block rate of 70080000", async () => {
          expect(await xvsVault.blocksOrSecondsPerYear()).equals(70080000);
        });
      });
