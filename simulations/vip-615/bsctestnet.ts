...
forking(91260874, async () => {
  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("market have correct owner", async () => {
      expect(await vXAUM.admin()).to.equal(bsctestnet.NORMAL_TIMELOCK);
    });

    it("market have correct ACM", async () => {
      expect(await vXAUM.accessControlManager()).to.equal(bsctestnet.ACCESS_CONTROL_MANAGER);
    });
    ...
    it("should not leave any vTokens in the timelock", async () => {
      const vXAUMTimelockBalance = await vXAUM.balanceOf(bsctestnet.NORMAL_TIMELOCK);
    ...
    it("vXAUM has flash loans enabled", async () => {
      const isFlashLoanEnabled = await vXAUM.isFlashLoanEnabled();
