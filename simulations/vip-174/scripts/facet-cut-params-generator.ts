...
const FacetNames = ["MarketFacet" as const, "PolicyFacet" as const, "RewardFacet" as const, "SetterFacet" as const];
...
async function generateCutParams() {
  ...
  function getFunctionSelector(selectors: string[]) {
  ...
  function makeCutParam(cut: { facetAddress: string; action: number; functionSelectors: string[] }[]) {
    const cutParams = [];
