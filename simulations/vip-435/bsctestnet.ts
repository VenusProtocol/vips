...
forking(47756644, async () => {
  ...
  testVip("THE market VIP", await vip440(), {});

  describe("Post-VIP behavior", async () => {
    it("has the correct price", async () => {
      expect(await resilientOracle.getPrice(marketSpec.vToken.underlying.address)).to.equal(parseUnits("0.6605", 18));
      expect(await resilientOracle.getUnderlyingPrice(marketSpec.vToken.address)).to.equal(parseUnits("0.6605", 18));
    });
