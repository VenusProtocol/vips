...
forking(39571070, async () => {
  ...
  testVip("vip-317 testnet", await vip317(), {
  ...
  describe("Post-VIP behavior", () => {
    it("Should match minDestGas value", async () => {
      expect(await bridge.minDstGasLookup(ARBITRUM_SEPOLIA_CHAIN_ID, 0)).to.equal(MIN_DST_GAS);
    });

    it("Should match single send transaction limit", async () => {
      expect(await bridge.chainIdToMaxSingleTransactionLimit(ARBITRUM_SEPOLIA_CHAIN_ID)).to.equal(SINGLE_SEND_LIMIT);
    });

    it("Should match single receive transaction limit", async () => {
      expect(await bridge.chainIdToMaxSingleReceiveTransactionLimit(ARBITRUM_SEPOLIA_CHAIN_ID)).to.equal(
        SINGLE_RECEIVE_LIMIT,
      );
    });

    it("Should match max daily send limit", async () => {
      expect(await bridge.chainIdToMaxDailyLimit(ARBITRUM_SEPOLIA_CHAIN_ID)).to.equal(MAX_DAILY_SEND_LIMIT);
    });

    it("Should match max daily receive limit", async () => {
      expect(await bridge.chainIdToMaxDailyReceiveLimit(ARBITRUM_SEPOLIA_CHAIN_ID)).to.equal(MAX_DAILY_RECEIVE_LIMIT);
    });
    ...
    it("Should emit an event on successfull bridging of XVS", async () => {
      ...
      await expect(
        ...
        .withArgs(ARBITRUM_SEPOLIA_CHAIN_ID, XVS_HOLDER, receiverAddressBytes32, amount);
