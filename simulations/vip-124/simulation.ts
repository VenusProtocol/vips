  ...
  describe("Post-VIP behavior", async () => {
    ...
    it("validate vToken prices", async () => {
      for (let i = 0; i < vTokens.length; i++) {
        ...
        if (vToken.oracle === "chainlink") {
          await chainlinkOracle.connect(signer).setTokenConfig({
            asset: vToken.assetAddress,
            feed: vToken.feed,
            maxStalePeriod: 60 * 60 * 24 * 7,
          });
        } else if (vToken.oracle === "binance") {
          await binanceOracle.connect(signer).setMaxStalePeriod(vToken.name, 60 * 60 * 24 * 7);
        }
