/**
 * =============================================================================
 * VIP-589 Test Suite - BNB Chain Mainnet
 * =============================================================================
 *
 * Overview:
 * VIP-589 adds the U market to the Stablecoins e-mode pool on BNB Chain and
 * also updates Prime rewards configuration and performs a one-off USDC → USDT
 * swap via the Prime Liquidity Provider and Protocol Share Reserve (PSR).
 ...
 *    - Verifies initial Prime configuration for USDT/USDC in PSR and PLP
 *
 * 2. VIP Execution
 *    - Executes VIP-589 proposal (which includes adding U to the Stablecoins
 *      e-mode pool and updating Prime/PSR configuration)
 *    - Verifies events:
 *      * PoolMarketInitialized
 *      * NewLiquidationIncentive
 *      * BorrowAllowedUpdated
 *      * DistributionConfigUpdated (PSR)
 *      * TokenDistributionSpeedUpdated (PrimeLiquidityProvider)
 *
 * 3. Post-VIP Behavior
 *    a) Market Configuration Tests
 *       - Verifies U market is correctly listed in pool with CF=0%, LT=0%
 *
 *    b) Prime Allocation & Swap Tests
 *       - Verifies new PSR percentage distributions for USDT/USDC converters
 *       - Verifies updated Prime reward distribution speeds
 *       - Asserts USDC balance decrease and USDT balance increase in PLP
 *       - Confirms NORMAL_TIMELOCK USDT balance remains within tolerance
 *
 *    c) Basic Operations Tests
 *       - Minting vU tokens
 *       - Entering U market
  ...
  describe("Post-VIP behavior", () => {
    describe("U-Market Emode", () => {
      ...
      it("should have U market listed in Stablecoins emode pool with correct configuration", async () => {
        ...
        expect(marketData.liquidationThresholdMantissa).to.equal(
          EMODE_POOL_SPECS.marketsConfig.vU.liquidationThreshold,
        );
        expect(marketData.liquidationIncentiveMantissa).to.equal(
