...
forking(57999649, async () => {
  ...
  before(async () => {
    xvsVault = await ethers.getContractAt(XVS_VAULT_ABI, OPBNBTESTNET_XVS_VAULT_PROXY);
    xvsVaultProxy = await ethers.getContractAt(PROXY_ABI, OPBNBTESTNET_XVS_VAULT_PROXY);
    vtokenBeacon = await ethers.getContractAt(VTOKEN_BEACON_ABI, OPBNBTESTNET_VTOKEN_BEACON);
    poolRegistry = await ethers.getContractAt(POOL_REGISTRY_ABI, NETWORK_ADDRESSES.opbnbtestnet.POOL_REGISTRY);
  });
