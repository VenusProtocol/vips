...
export const getConverterPermissions = (converter: string): string[][] => {
  return [
    ...accounts.map(account => [converter, "pauseConversion()", account]),
    ...accounts.map(account => [converter, "resumeConversion()", account]),
    ...timelocks.map(account => [converter, "setMinAmountToConvert(uint256)", account]),
    ...timelocks.map(account => [converter, "setConversionConfig(address,address,ConversionConfig)", account]),
    ...timelocks.map(account => [converter, "setAssetsDirectTransfer(address[],bool[])", account]),
  ];
};
...
export function splitPermissions(
